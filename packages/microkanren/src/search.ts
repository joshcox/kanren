import { cons, force, future, isAbort, isCons, isEmpty, isFuture, isLazy, isStream, lazy, mapFuture, stream, Stream, StreamKinds } from "./data/stream";

export { take as takeUntil } from './data/stream';

export const unit = {
    stream: <A>(items: A[]): Stream<A> => stream(items)
};
...
export const plus = <A>($1: Stream<A>, $2: Stream<A>): Stream<A> => {
    if (isStream($1) && isStream($2)){
        ...
        else if (isCons<A, Stream<A>, StreamKinds>($1)) {
...
export const bind = <A>(g: (item: A) => Stream<A>, $: Stream<A>): Stream<A> => {
    if (isStream($)) {
        if (isAbort($) || isEmpty($)) return $;
        else if (isLazy($)) return bind(g, force($));
        else if (isFuture($)) return mapFuture($, (s) => bind(g, s));
        else if (isCons<A, Stream<A>, StreamKinds>($)) return plus(bind(g, $.cdr), g($.car));
